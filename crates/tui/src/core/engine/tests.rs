...
fn internal_context_budget_unaffected_by_api_request_cap() {
    ...
    // Sub-500K models cross into the effective-cap branch: a 256K self-hosted
    // deployment must yield a usable budget rather than None — the prior
    // implementation of context_input_budget computed 256K − 262K − 1K =
    // negative for any sub-500K window, silently disabling preflight checks.
    let small_window_budget = context_input_budget("qwen36_35b_256k")
        .expect("256K-suffix model should yield Some budget under the effective-cap branch");
    let effective_output = effective_max_output_tokens("qwen36_35b_256k") as usize;
