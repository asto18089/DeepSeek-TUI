...
async fn run_subagent(
    ...
    let transcript_role = assignment.role.clone().unwrap_or_else(|| agent_type.as_str().to_string());
                ...
                if output_schema.is_some() && output_submitted.is_none() {
                    ...
                    if structured_retries < MAX_STRUCTURED_OUTPUT_RETRIES {
                        ...
                        final_result = Some(format!(
                            ...
                            last_structured_error.as_deref().unwrap_or("(模型始终未调用 submit_output)")
                        ));
                        break;
                    }
                ...
                match validate_against_schema(&tool_input, schema) {
                    Ok(()) => match persist_structured_output(&project_dir, &tool_input, schema) {
                        ...
                        Ok(_) => {
                            ...
                            format!("Error: 产出未落盘任何文件(第 {structured_retries}/{MAX_STRUCTURED_OUTPUT_RETRIES} 次): {e}")
                        }
                        Err(write_err) => {
                            structured_retries += 1;
